...
for i in range(14):
    ts = base.query('select 申请时间    from Table1 order by 申请时间 desc limit 0,1')[0]['申请时间']
    def encrypt():
        hl     = hashlib.md5()
        print(type(ts))
    ...
    user_data = {
        'passwd'           : passwd,
        'application_time' : ts,
        'zh_name'          : base.query('select 中文姓名     from Table1 order by 申请时间 desc limit 0,1')[0]['中文姓名'],
